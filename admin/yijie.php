...
<?php
	$ordersql = "select * from w_order left join w_courier on w_courier.courier_id=w_order.courier_id left join w_user on w_user.user_openid=w_order.user_openid where (w_order.order_static=1) order by order_time desc limit ".($pagenum*20).",20";
	...
	if($ordernum != 0){
		...
		while($orderow=mysqli_fetch_array($orderult)){
			...
			echo "<tr><td align='center' style='width:300px;height:30px;overflow:hidden;'>".$order_no."</td>";
...
//			echo "<td align='center'>".$user_openid."</td>";
	...
	if($pagenum == 0){
		...
		echo "<a href='yijie.php?pagenum=".($pagenum+1)."' target='mainframe'><span style='margin-left:6rem;border:1px #ff7575 solid;font-size:1rem;pdding:1rem;'>下一页</span></a>";
	}else{
		echo "<a href='yijie.php?pagenum=".($pagenum-1)."' target='mainframe'><span style='border:1px #ff7575 solid;font-size:1rem;pdding:1rem;'>上一页</span></a>";
		echo "<a href='yijie.php?pagenum=".($pagenum+1)."' target='mainframe'><span style='margin-left:6rem;border:1px #ff7575 solid;font-size:1rem;pdding:1rem;'>下一页</span></a>";
	}
	echo "</p>";
?>
