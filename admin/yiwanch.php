 		...
 		<div class="zhifu-contanier" style="width:100%;margin-top:-4rem;background:#fff;" >
    		<div class="weijie-list" style="width:100%; margin:0 auto;">
    			<div style="width:100%;margin-top:1rem;margin-left:1rem;padding:1rem;" align="left">
    				...
    				<span>订单管理&nbsp>>>&nbsp已完成订单浏览</span>
    			</div>
                <div class="lidt-infor" style="margin-left:1rem;">
                	<table class="first-list" border="1" cellspacing="0" cellpadding="0">
                    	<tr>
                        	...
                        	<th style="width:240px;" align="center">订单详情</th>
...
<?php
	$ordersql = "select * from w_order_over left join w_courier on w_courier.courier_id=w_order_over.courier_id left join w_user on w_user.user_openid=w_order_over.user_openid where (w_order_over.order_static=2) order by order_time desc limit ".($pagenum*20).",20";
	...
	if($ordernum != 0){
		...
		while($orderow=mysqli_fetch_array($orderult)){
			...
			echo "<td align='left' title='".$order_dtl."' style='width:240px;display:block;overflow:hidden;white-space: nowrap;text-overflow: ellipsis;'>".$order_dtl."</td>";
...
//			echo "<td align='center'>".$user_openid."</td>";
			...
			echo "<td align='left' title='".$order_adrs."' style='width:300px;overflow:hidden;display:block;white-space: nowrap;text-overflow: ellipsis;'>".$order_adrs."</td>";
	...
	mysqli_free_result($orderult);
			...
			if($pagenum == 0){
				...
				echo "<a href='yiwanch.php?pagenum=".($pagenum+1)."' target='mainframe'><span style='margin-left:6rem;border:1px #ff7575 solid;font-size:1rem;pdding:1rem;'>下一页</span></a>";
			}else{
				echo "<a href='yiwanch.php?pagenum=".($pagenum-1)."' target='mainframe'><span style='border:1px #ff7575 solid;font-size:1rem;pdding:1rem;'>上一页</span></a>";
				echo "<a href='yiwanch.php?pagenum=".($pagenum+1)."' target='mainframe'><span style='margin-left:6rem;border:1px #ff7575 solid;font-size:1rem;pdding:1rem;'>下一页</span></a>";
			}
			echo "</p>";
?>
